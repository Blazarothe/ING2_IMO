...
$sql = "SELECT lname, fname, email FROM users WHERE user_id=?";
...
if ($typeUtilisateur === 'client') {
    ...
    $agentSql = "SELECT a.id, u.lname, u.fname FROM Agents a JOIN users u ON a.utilisateur_id = u.user_id";
    $agentResult = $conn->query($agentSql);
}

// Récupérer les rendez-vous pour l'utilisateur ou l'agent
$rdvSql = ($typeUtilisateur == 'client') ? 
    "SELECT rv.date_heure, p.adresse, u.lname AS agent_nom, u.fname AS agent_prenom FROM RendezVous rv JOIN Proprietes p ON rv.propriete_id = p.propriete_id JOIN users u ON rv.agent_id = u.user_id WHERE rv.client_id = ? AND rv.statut = 'confirmé'" :
    "SELECT rv.date_heure, p.adresse, u.lname AS client_nom, u.fname AS client_prenom FROM RendezVous rv JOIN Proprietes p ON rv.propriete_id = p.propriete_id JOIN users u ON rv.client_id = u.user_id WHERE rv.agent_id = ? AND rv.statut = 'confirmé'";
    ...
    <main>
        ...
        <div class="form-container">
            <form method="post" action="compte.php">
                ...
                <input type="text" id="nom" name="nom" value="<?= htmlspecialchars($user['lname']) ?>" required>

                <label for="prenom">Prénom :</label>
                <input type="text" id="prenom" name="prenom" value="<?= htmlspecialchars($user['fname']) ?>" required>
                ...
                <form method="post" action="compte.php">
                    ...
                    <select id="agent_id" name="agent_id" required>
                        <?php while ($row = $agentResult->fetch_assoc()): ?>
                            <option value="<?= htmlspecialchars($row['id']) ?>"><?= htmlspecialchars($row['fname'] . ' ' . $row['lname']) ?></option>
                        <?php endwhile; ?>
                    </select>
